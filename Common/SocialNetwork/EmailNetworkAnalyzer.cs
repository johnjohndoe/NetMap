...
public class EmailNetworkAnalyzer : Object
{
    ...
    ///
    /// <returns>
    /// An array of zero or more <see cref="ParticipantPair" /> objects, one
	/// for each pair of participants in the user's email social network.  The
	...
	///
	/// <para>
	/// The aggregated set of <see cref="ParticipantPair" /> objects is
	/// returned.
	/// </para>
	///
	/// <para>
    ...
    /// <returns>
    /// An array of zero or more <see cref="ParticipantPair" /> objects, one
	/// for each pair of participants in the user's email social network that
	/// satisfy the specified criteria.  The return value is never null.
    /// </returns>
    ///
    /// <remarks>
	...
	/// will return an array of zero or more <see cref="ParticipantPair" />
	...
	)
    {
		...
		ParticipantPair [] aoParticipantPairs = null;

		try
		{
			aoParticipantPairs = AnalyzeEmailNetworkInternal(
		...
		if ( !String.IsNullOrEmpty(participant1) )
		{
			...
			// ParticipantPair.Participant1 slot for each element in the
			// returned array that includes the first participant.

            String sParticipant1Lower = participant1.ToLower();

			foreach (ParticipantPair oParticipantPair in aoParticipantPairs)
			{
				if (oParticipantPair.Participant2 == sParticipant1Lower)
				{
					// Swap the participants.

					String sTemp = oParticipantPair.Participant1;

					oParticipantPair.Participant1 =
						oParticipantPair.Participant2;

					oParticipantPair.Participant2 = sTemp;
				}
			}
		}

		return (aoParticipantPairs);
    }
    ...
    /// <returns>
    /// An array of zero or more <see cref="ParticipantPair" /> objects, one
	/// for each pair of participants in the set of email items.  The return
	/// value is never null.
    /// </returns>
    //*************************************************************************
	...
	)
    {
		...
		// Convert the aggregated results to an array of ParticipantPair
		// objects.

		ParticipantPair [] aoParticipantPairs =
			new ParticipantPair[oAggregatedDictionary.Count];
			...
			oAggregatedDictionary)
		{
			...
			aoParticipantPairs[i] = new ParticipantPair(
	...
	)
	{
		...
		ParticipantPair [] aoParticipantPairs = AnalyzeEmailNetworkInternal(
