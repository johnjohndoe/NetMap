...
public partial class PajekGraphAdapter : GraphAdapterBase, IGraphAdapter
{
			...
			if ( sLine.StartsWith("*") )
			{
				...
				if ( sLine.StartsWith("*vertices") )
				{
					...
					if (asFields.Length != 2 ||
						!Int32.TryParse(asFields[1], out iVerticesExpected) ||
						iVerticesExpected < 0)
					{
	...
	)
	{
		...
		if (!oMatch.Success
			||
			!Int32.TryParse(oMatch.Groups["VertexNumber"].Value,
				out iVertexNumber)
			)
		{
		...
		if (sX.Length > 0)
		{
			...
			if ( !Single.TryParse(sX, out fX) ||
				!Single.TryParse(oMatch.Groups["Y"].Value, out fY) ||
				fX < 0 || fX > 1.0F ||
				fY < 0 || fY > 1.0F
				)
			{
	...
	)
	{
		...
		if (asFields.Length < 3 ||
			!Int32.TryParse(asFields[0], out iFirstVertexNumber) ||
			!Int32.TryParse(asFields[1], out iSecondVertexNumber) ||
			!Single.TryParse(asFields[2], out fWeight)
			)
		{
			OnLoadFormatError(sLine, iLineNumber, ExpectedFormat);
		}
	...
	)
	{
		...
		for (Int32 i = 0; i < iFields; i++)
		{
			...
			if ( !Int32.TryParse(asFields[i], out iVertexNumber) )
			{
				OnLoadFormatError(sLine, iLineNumber, ExpectedFormat);
			}
    ...
    )
	{
			...
			)
		{
			throw new InvalidOperationException( String.Format(
				...
				,
				oEdge.ID.ToString(NetMapBase.Int32Format)
				) );
		}
