...
/// cref="NetMapControl" />.  It provides a <see cref="Layout" /> property for
/// keeping track of the layout currently in use, a <see
/// cref="LayoutChanged" /> event that is raised when the layout is changed,
/// and an <see cref="ApplyLayoutToNetMapControl" /> method for setting the <see
/// cref="NetMapControl.Layout" />, <see cref="NetMapControl.VertexDrawer" />,
/// and <see cref="NetMapControl.EdgeDrawer" /> properties on a <see
...
public class LayoutManager : Object
{
    ...
    //*************************************************************************
    //  Method: ApplyLayoutToNetMapControl()
    //
    /// <summary>
	/// Sets the layout-related properties on a <see cref="NetMapControl" />.
    /// </summary>
    ///
	...
	///
	/// <remarks>
	/// This method sets the <see cref="NetMapControl.Layout" />, <see
	/// cref="NetMapControl.VertexDrawer" />, and <see
	/// cref="NetMapControl.EdgeDrawer" /> properties on <paramref
	/// name="netMapControl" /> to a set of objects that are designed to work
	/// together to render a layout of type <see cref="Layout" />.
	///
	/// <para>
	/// Important: The call to this method must be surrounded by <see
	/// cref="NetMapControl.BeginUpdate" /> and <see
	/// cref="NetMapControl.EndUpdate()" /> to avoid unwanted graph redraws.
	/// </para>
	///
	/// </remarks>
    //*************************************************************************
    ...
    public void
    ApplyLayoutToNetMapControl
	(
		NetMapControl netMapControl,
		Int32 margin
	)
	{
		const String MethodName = "ApplyLayoutToNetMapControl";

		this.ArgumentChecker.CheckArgumentNotNull(
			MethodName, "netMapControl", netMapControl);
		...
		netMapControl.Layout = oLayout;
		netMapControl.VertexDrawer = oVertexDrawer;
		netMapControl.EdgeDrawer = oEdgeDrawer;
    }
