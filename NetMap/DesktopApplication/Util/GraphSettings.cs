...
public class GraphSettings : ApplicationSettingsBase
{
    ...
    )
    {
		...
		IEdgeDrawer oEdgeDrawer1 = netMapControl.EdgeDrawer;

		if ( !(oEdgeDrawer1 is EdgeDrawer) )
		{
			throw new InvalidOperationException( String.Format(

				"{0}.{1}: NetMapControl.EdgeDrawer was set to an edge drawer"
		...
		IVertexDrawer oVertexDrawer1 = netMapControl.VertexDrawer;

		if ( !(oVertexDrawer1 is VertexDrawer) )
		{
			throw new InvalidOperationException( String.Format(

				"{0}.{1}: NetMapControl.VertexDrawer was set to a vertex drawer"
				+ " that is not supported."
				,
				this.ClassName,
				MethodName
		...
		netMapControl.BackColor = this.BackColor;
