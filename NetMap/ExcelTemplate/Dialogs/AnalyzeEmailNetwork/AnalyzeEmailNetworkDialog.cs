...
public partial class AnalyzeEmailNetworkDialog : ExcelTemplateForm
{
	...
	{
		Debug.Assert(aoParticipantPairs != null);
		Debug.Assert(m_oEdgeTable != null);
		AssertValid();

		Int32 iParticipantPairs = aoParticipantPairs.Length;
		...
		// Create and populate arrays of vertex names and tie strengths.

		Object [,] aoVertex1Names = new Object [iParticipantPairs, 1];
		Object [,] aoVertex2Names = new Object [iParticipantPairs, 1];
		Object [,] aoTieStrengths = new Object [iParticipantPairs, 1];

		for (Int32 i = 0; i < iParticipantPairs; i++)
		{
			ParticipantPair oParticipantPair = aoParticipantPairs[i];

			aoVertex1Names[i, 0] =
				AnalyzerToParticipant(oParticipantPair.Participant1);

			aoVertex2Names[i, 0] =
				AnalyzerToParticipant(oParticipantPair.Participant2);

			aoTieStrengths[i, 0] = oParticipantPair.TieStrength.ToString();
		}

		// Write the arrays to the edge table.

		Range oVertex1ColumnData;
		
		Boolean bFound = ExcelUtil.TryGetTableColumnData(m_oEdgeTable,
			EdgeTableColumnNames.Vertex1Name, out oVertex1ColumnData);

		Debug.Assert(bFound);

		ExcelUtil.SetRangeValues(oVertex1ColumnData, aoVertex1Names);

		Range oVertex2ColumnData;
		
		bFound = ExcelUtil.TryGetTableColumnData(m_oEdgeTable,
			EdgeTableColumnNames.Vertex2Name, out oVertex2ColumnData);

		Debug.Assert(bFound);

		ExcelUtil.SetRangeValues(oVertex2ColumnData, aoVertex2Names);
	...
	)
	{
        ...
        {
			// Clear the required edge table and the optional vertex table.

			ExcelUtil.ClearTable(m_oEdgeTable);
			...
			{
				ExcelUtil.ClearTable(oVertexTable);
			}

			// Populate the edge table with participant pairs.

            Debug.Assert(e.Result is ParticipantPair[]);

            ParticipantPair[] aoParticipantPairs =
				( ParticipantPair[] )e.Result;


			if (aoParticipantPairs.Length > 0)
			{
				PopulateEdgesTable(aoParticipantPairs);
			}
			else
			{
	...
	protected const String HelpMessage =
		...
		+ " in this case."
		+ "\r\n\r\n"
		+ "If you read the workbook into the .NetMap graph, you'll see a"
