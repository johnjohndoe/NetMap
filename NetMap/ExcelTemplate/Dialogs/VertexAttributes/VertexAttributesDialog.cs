...
///
/// <remarks>
/// Pass the NetMapControl to the constructor.  If the user performs any edits
/// and the edits don't require that the workbook be read again, this dialog
/// applies the edits to the NetMapControl's selected vertices and returns
...
public partial class VertexAttributesDialog : ExcelTemplateForm
{
	...
	/// NetMapControl whose vertex attributes need to be edited.
	/// </param>
	//*************************************************************************
	...
	this()
	{
		...
		m_oNetMapControl = netMapControl;
    ...
    //
    /// <summary>
    /// Gets the attributes that were applied to the NetMapControl's selected
	/// vertices.
    /// </summary>
    ///
    /// <value>
    /// The attributes that were applied to the NetMapControl's selected
	/// vertices.
    /// </value>
	///
	/// <remarks>
	...
	where T : struct
	{
		Debug.Assert(m_oNetMapControl != null);
		// AssertValid();
		...
		IVertex [] aoSelectedVertices = m_oNetMapControl.SelectedVertices;
	...
	)
    {
		...
		if (!m_oEditedVertexAttributes.WorkbookMustBeReread)
		{
			...
			foreach (IVertex oVertex in m_oNetMapControl.SelectedVertices)
			{
				if (bApplyColor)
				{
				...
				if (bApplyVisibility)
				{
					...
					{
						oIncidentEdge.SetValue(
							ReservedMetadataKeys.Hide, null);
					}
				}
	...
	protected NetMapControl m_oNetMapControl;
