...
// This class inherits from NetMapBase to get the extended ToString()
// functionality.

public class Community : NetMapBase
{
    //*************************************************************************
	...
	OnMergedCommunities
	(
		Community oMergedCommunity1,
		Community oMergedCommunity2,
		Community oNewMergedCommunity,
		Single fNewCommunityPairDeltaQ,
		DeltaQMaxHeap oDeltaQMaxHeap
	)
	{
		Debug.Assert(oMergedCommunity1 != null);
		Debug.Assert(oMergedCommunity2 != null);
		Debug.Assert(oNewMergedCommunity != null);
		Debug.Assert(oDeltaQMaxHeap != null);
		AssertValid();

		Single fPreviousCommunityPairDeltaQ = Single.MinValue;
		Int32 iMergedCommunity1ID = oMergedCommunity1.ID;
		Int32 iMergedCommunity2ID = oMergedCommunity2.ID;
		...
		for (Int32 i = m_oCommunityPairs.Count - 1; i >= 0; i--)
		{
			...
			if (iOtherCommunityID == iLargerMergedCommunityID)
			{
				// This community pair connects to oMergedCommunity2.
			...
			{
				// This community pair connects to oMergedCommunity1.
		...
		oNewCommunityPair.Community2 = oNewMergedCommunity;
		oNewCommunityPair.DeltaQ = fNewCommunityPairDeltaQ;
        m_oCommunityPairs.Add(oNewMergedCommunity.ID, oNewCommunityPair);
		...
		if (fPreviousCommunityPairDeltaQ < fOldMaximumDeltaQ)
		{
			...
			if (fNewCommunityPairDeltaQ <= fPreviousCommunityPairDeltaQ)
			{
			...
			else
			{
				...
				if (fNewCommunityPairDeltaQ > fOldMaximumDeltaQ)
				{
		...
		if (fOldMaximumDeltaQ != this.MaximumDeltaQ)
		{
			...
			oDeltaQMaxHeap.UpdateValue(this, this.MaximumDeltaQ);
		}
	}
	...
	/// SortedList of CommunityPair objects.  The sorted key is the
	/// CommunityPair.ID and the value is the CommunityPair.
