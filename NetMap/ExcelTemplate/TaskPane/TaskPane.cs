...
//
/// <summary>
/// User control that implements the NetMap custom task pane.
/// </summary>
///
/// <remarks>
/// This Excel 2007 custom task pane contains a NetMapControl that displays a
/// NetMap graph, along with controls for manipulating the graph.  It utilizes
...
public partial class TaskPane : UserControl
{
    ...
    //
    /// <summary>
	/// Initializes the task pane's NetMapControl.
    /// </summary>
    //*************************************************************************
	...
	InitializeNetMapControl()
	{
		Debug.Assert(oNetMapControl != null);
		Debug.Assert(!oNetMapControl.IsDrawing);
		AssertValid();
    ...
    //
    /// <summary>
	/// Transfers data from the workbook to the NetMapControl and refreshes the
	/// NetMapControl.
    /// </summary>
    //*************************************************************************
	...
	ReadWorkbook()
	{
		...
		// edits vertex attributes in the NetMap graph.  (If the vertex
		...
		try
		{
			...
			// Load the NetMapControl with the resulting graph.
			...
			oNetMapControl.ShowToolTips = oReadWorkbookContext.ToolTipsUsed;

			// Apply the current layout to the NetMapControl.
    ...
    //
    /// <summary>
	/// Forces the NetMapControl to lay out and draw the graph again.
    /// </summary>
    //*************************************************************************
    ...
    //
    /// <summary>
	/// Forces the NetMapControl to lay out selected vertices in the graph
	/// again.
    /// </summary>
    //*************************************************************************
	...
	ForceLayoutSelected()
    {
		...
		// oNetMapControl_GraphDrawn() removes the value.

		oNetMapControl.Graph.SetValue(
			ReservedMetadataKeys.LayOutTheseVerticesOnly,
            oNetMapControl.SelectedVertices);
	...
	ShowGeneralUserSettingsDialog()
    {
		...
		{
			// Apply the new settings and current layout to the NetMapControl.
	...
	//
	/// <summary>
	/// Applies the current general settings and layout to the NetMapControl.
	/// </summary>
	///
	/// <remarks>
	/// Important: The call to this method must be surrounded by
	/// NetMapControl.BeginUpdate and NetMapControl.EndUpdate to avoid unwanted
	/// graph redraws.
	/// </remarks>
	//*************************************************************************
    ...
    //
    /// <summary>
	/// Applies the current layout to the NetMapControl.
    /// </summary>
    ///
	/// <remarks>
	/// Important: The call to this method must be surrounded by
    ...
    ApplyLayout()
	{
		...
		m_oLayoutManagerForMenu.ApplyLayoutToNetMapControl(
			oNetMapControl, m_oGeneralUserSettings.Margin);
    }
	...
	//
	/// <summary>
	/// Applies the current general settings to the NetMapControl.
	/// </summary>
	///
	/// <remarks>
	/// Important: The call to this method must be surrounded by
	...
	ApplyUserSettings()
    {
		...
		m_oGeneralUserSettings.TransferToNetMapControl(oNetMapControl);
    }
    ...
    //
    /// <summary>
	/// Enables the menu items on the the cmsNetMapControl context menu.
    /// </summary>
    ///
	...
	)
    {
		...
		ToolStripItemCollection oItems = cmsNetMapControl.Items;
		...
		Int32 iVertices = oNetMapControl.Graph.Vertices.Count;
		IVertex [] aoSelectedVertices = oNetMapControl.SelectedVertices;
		...
		if (iSelectedVertices == 1)
		{
					...
					out oCustomMenuItemInformationAsObject) )
			{
				...
				if (aoCustomMenuItemInformation.Length > 0)
				{
					...
					cmsNetMapControl.Items.Add(oToolStripSeparator);
				}
					...
					aoCustomMenuItemInformation)
				{
					...
					cmsNetMapControl.Items.Add(oToolStripMenuItem);
	...
	)
	{
		...
		Int32 iVertices = oNetMapControl.Graph.Vertices.Count;
		Int32 iSelectedVertices = oNetMapControl.SelectedVertices.Length;
	...
	)
	{
		...
		Int32 iEdges = oNetMapControl.Graph.Edges.Count;
		Int32 iSelectedEdges = oNetMapControl.SelectedEdges.Length;
	...
	)
	{
		...
		oNetMapControl.SetSelected(

			bSelect ? NetMapControlUtil.GetVerticesAsArray(oNetMapControl) :
				new IVertex[0],

			oNetMapControl.SelectedEdges
			);
	}
	...
	)
	{
		...
		oNetMapControl.SetSelected(
			...
			bSelect ? NetMapControlUtil.GetEdgesAsArray(oNetMapControl) :
				new IEdge[0]
			);
	}
	...
	)
	{
		...
		Dictionary<IEdge, Char> oSelectedEdges =
			NetMapControlUtil.GetSelectedEdgesAsDictionary(oNetMapControl);
		...
		oNetMapControl.SetSelected(oNetMapControl.SelectedVertices,
	...
	)
	{
		...
		Dictionary<IVertex, Char> oSelectedVertices =
			NetMapControlUtil.GetSelectedVerticesAsDictionary(oNetMapControl);
	...
	)
	{
		...
		SelectSubgraphsDialog oSelectSubgraphsDialog =
			new SelectSubgraphsDialog(oNetMapControl, oClickedVertex);
	...
	EditVertexAttributes()
    {
		...
		VertexAttributesDialog oVertexAttributesDialog =
			new VertexAttributesDialog(oNetMapControl);
        ...
        List<Int32> oSelectedVertexIDs =
			NetMapControlUtil.GetSelectedVertexIDsAsList(oNetMapControl);
    ...
    //
    /// <summary>
	/// Handles the Click event for custom menu items on the cmsNetMapControl
	/// ContextMenuStrip.
    /// </summary>
    ///
	/// <param name="sender">
    ...
    //*************************************************************************
    //  Method: oNetMapControl_GraphMouseUp()
    //
    /// <summary>
	/// Handles the GraphMouseUp event on the oNetMapControl control.
    /// </summary>
    ///
	/// <param name="sender">
	/// Standard event argument.
	...
	)
    {
		...
		cmsNetMapControl.Show( oNetMapControl.PointToScreen(
	...
	)
    {
		...
		oNetMapControl.Graph.RemoveKey(
			ReservedMetadataKeys.LayOutTheseVerticesOnly);

		// If the edge and vertex dictionaries are null, NetMapControl has drawn
			...
			if (oGraphDrawn != null)
			{
				...
				try
				{
					oGraphDrawn(this, new GraphDrawnEventArgs(
						oNetMapControl.ClientRectangle, m_oEdgeIDDictionary,
    ...
    //*************************************************************************
    //  Method: oNetMapControl_VertexMoved()
    //
    /// <summary>
	/// Handles the VertexMoved event on the oNetMapControl control.
    /// </summary>
    ///
	/// <param name="sender">
	/// Standard event argument.
			...
			try
			{
				oVertexMoved( this,
					new VertexMovedEventArgs(oVertex, (Int32)oVertex.Tag,
						oNetMapControl.ClientRectangle) );
			}
			catch (Exception oException)
			{
				ErrorUtil.OnException(oException);
	...
	)
    {
		...
		List<Int32> oSelectedEdgeIDs =
			NetMapControlUtil.GetSelectedEdgeIDsAsList(oNetMapControl);
	...
	)
	{
		...
		else
		{
			...
			{
				oEdgesToSelect =
					NetMapControlUtil.GetSelectedEdgesAsDictionary2(
						oNetMapControl);
			}
			else if (e.SelectedEdgeIDs.Length > 0)
			{
				oVerticesToSelect =
					NetMapControlUtil.GetSelectedVerticesAsDictionary2(
						oNetMapControl);
			}
			else
			{
