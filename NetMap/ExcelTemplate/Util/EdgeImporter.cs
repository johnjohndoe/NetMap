...
/// to the edge worksheet of a NetMap workbook.
/// </remarks>
//*****************************************************************************
...
public class EdgeImporter : Object
{
    ...
    /// </param>
    ///
    /// <param name="destinationNetMapWorkbook">
	/// NetMap workbook the edge columns will be imported to.
    /// </param>
    ///
    /// <remarks>
	...
	/// <paramref name="destinationNetMapWorkbook" />.  If the columns can't be
    ...
    (
        ...
        Microsoft.Office.Interop.Excel.Workbook destinationNetMapWorkbook
    )
	{
		...
		Debug.Assert(destinationNetMapWorkbook != null);
		AssertValid();

		// Get the active worksheet of the source workbook.

		Application oApplication = destinationNetMapWorkbook.Application;
		...
		// Get the edge table in the destination NetMap workbook.

		ListObject oDestinationEdgeTable = GetDestinationEdgeTable(
			destinationNetMapWorkbook);

        ExcelUtil.ActivateWorksheet(oDestinationEdgeTable);

		ExcelUtil.ClearTable(oDestinationEdgeTable);

		// Copy the vertex columns to the destination NetMap workbook.
    ...
    //
    /// <summary>
	/// Gets the edge table in the destination NetMap workbook.
    /// </summary>
    ///
    /// <param name="oDestinationNetMapWorkbook">
	/// NetMap Workbook containing the edge table.
    /// </param>
	///
	/// <returns>
	/// The edge table.
	...
	(
        Microsoft.Office.Interop.Excel.Workbook oDestinationNetMapWorkbook
	)
	{
		Debug.Assert(oDestinationNetMapWorkbook != null);

		EdgeWorksheetReader oEdgeWorksheetReader = new EdgeWorksheetReader();

		return ( oEdgeWorksheetReader.GetEdgeTable(
			oDestinationNetMapWorkbook) );
	}
	...
	/// Edge table in the destination NetMap workbook.
    /// </param>
    ///
	...
	)
	{
		...
		{
			throw new ImportEdgeException(
				"One of the edge columns is missing from the .NetMap workbook."
				);
		}
	...
	)
	{
			...
			oOneBasedColumnNumbersToImport)
		{
			...
			if (bSourceColumnsHaveHeaders)
			{
				...
				{
					// The source column name is one of NetMap's reserved
