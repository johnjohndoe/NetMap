...
public class EdgeCollectionTest : Object
{
    ...
    TestConstructor()
    {
		...
		Assert.IsFalse(m_oEdgeCollection.IsSynchronized);
		Assert.AreEqual(m_oEdgeCollection, m_oEdgeCollection.SyncRoot);
    }
    ...
    TestAdd7()
    {
		...
		IEdge oEdge = oEdgeFactory.CreateEdge(
			aoVertices[0], aoVertices[1], true);
		...
		oEdge = oEdgeFactory.CreateEdge(
			aoVertices[1], aoVertices[0], true);
    ...
    TestAddBad7()
    {
		...
		try
		{
			...
			IVertex oNonContainedVertex =
				( new VertexFactory() ).CreateVertex();
    ...
    TestAddBad13()
    {
		...
		try
		{
			...
			oEdge = ( new EdgeFactory() ).CreateEdge(
				aoVertices[0], aoVertices[1], false);
    ...
    TestAddBad14()
    {
		...
		try
		{
			...
			IEdge oEdge = ( new EdgeFactory() ).CreateEdge(
				aoVertices[0], aoVertices[0], false);
    ...
    //
    /// <summary>
    /// Tests the Add(IEdgeFactory, IVertex, IVertex, Boolean) method.
    /// </summary>
    //*************************************************************************
    ...
    TestAdd2_()
    {
		...
		AddEdges(100, GraphDirectedness.Undirected, AddOverload.IEdgeFactory);
    }
    ...
    TestAdd2_2()
    {
		...
		AddEdges(100, GraphDirectedness.Directed, AddOverload.IEdgeFactory);
    }
    ...
    TestAdd2_3()
    {
		...
		AddEdges(100, GraphDirectedness.Mixed, AddOverload.IEdgeFactory);
    }
    ...
    TestAdd2_Bad1()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(null, aoVertices[0], aoVertices[1], false);
		}
		catch (ArgumentNullException oArgumentNullException)
		{
			Assert.AreEqual(
				...
				"Microsoft.NetMap.Core."
				+ "EdgeCollection.Add: edgeFactory argument can't be null.\r\n"
				+ "Parameter name: edgeFactory"
				,
				oArgumentNullException.Message
				);
    ...
    TestAdd2_Bad2()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), null, aoVertices[0], false);
		}
		catch (ArgumentNullException oArgumentNullException)
		{
			Assert.AreEqual(
    ...
    TestAdd2_Bad3()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), aoVertices[0], null, false);
		}
		catch (ArgumentNullException oArgumentNullException)
		{
			Assert.AreEqual(
    ...
    TestAdd2_Bad4()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), oNonContainedVertex, aoVertices[0], false);
		}
		catch (ArgumentException oArgumentException)
		{
			Assert.AreEqual(
    ...
    TestAdd2_Bad5()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), aoVertices[0], oNonContainedVertex, false);
		}
		catch (ArgumentException oArgumentException)
		{
			Assert.AreEqual(
    ...
    TestAdd2_Bad6()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), oWrongTypeVertex, aoVertices[0], false);
		}
		catch (ArgumentException oArgumentException)
		{
            Assert.AreEqual(
    ...
    TestAdd2_Bad7()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), aoVertices[0], oWrongTypeVertex, false);
		}
		catch (ArgumentException oArgumentException)
		{
            Assert.AreEqual(
    ...
    TestAdd2_Bad8()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), aoVertices[0], aoVertices[1], false);
		}
		catch (ArgumentException oArgumentException)
		{
			Assert.AreEqual(
    ...
    TestAdd2_Bad9()
    {
		...
		try
		{
			...
			m_oEdgeCollection.Add(
				new EdgeFactory(), aoVertices[0], aoVertices[1], true);
		}
		catch (ArgumentException oArgumentException)
		{
			Assert.AreEqual(
    ...
    TestCopyToBad2()
    {
		...
		try
		{
			...
			m_oEdgeCollection.CopyTo(aiInt, 0);
		}
		catch (ArgumentException oArgumentException)
		{
			Assert.AreEqual(
				...
				"Microsoft.NetMap.Core."
				+ "EdgeCollection.CopyTo: array is not of type IEdge[].\r\n"
				+ "Parameter name: array"
				,
				oArgumentException.Message
				);
    ...
    TestRemove()
    {
		...
		IEdge oNonContainedEdge =
			oEdgeFactory.CreateEdge(aoVertices[0], aoVertices[1], false);
    ...
    TestGetConnectingEdges5()
    {
		...
		for (Int32 i = 0; i < Vertices; i++)
		{
			...
			IEdge oSelfLoopEdge = aoSelfLoopEdges[i] =
				oEdgeFactory.CreateEdge(oVertex, oVertex, false);
    ...
    TestToString4()
    {
		...
		Assert.AreEqual(
			"0 edges",
			m_oEdgeCollection.ToString(null)
			);
    }
    ...
    TestToString5()
    {
		...
		Assert.AreEqual(
			"0 edges",
			m_oEdgeCollection.ToString(String.Empty)
			);
    }
    ...
    TestToString8()
    {
		...
		String sExpectedValue =
			...
			+ "\tID = " + aoEdges[2].ID.ToString(NetMapBase.Int32Format)
			+ "\r\n"

			+ "\tID = " + aoEdges[1].ID.ToString(NetMapBase.Int32Format)
			+ "\r\n"

			+ "\tID = " + aoEdges[0].ID.ToString(NetMapBase.Int32Format)
			+ "\r\n"
			;

		Assert.AreEqual(
			sExpectedValue,
			m_oEdgeCollection.ToString("D")
			);
    }
    ...
    TestToStringBad()
    {
		...
		try
		{
			m_oEdgeCollection.ToString("Bad");
		}
		catch (FormatException oFormatException)
		{
			Assert.AreEqual(
				...
				"Microsoft.NetMap.Core."
				+ "EdgeCollection.ToString: Invalid format.  Available formats"
				+ " are G, P, and D."
				,
    ...
    AddOverload
    {
        ...
        /// <summary>
        /// Call Add(IEdgeFactory, IVertex, IVertex, Boolean).
        /// </summary>
	...
	)
    {
		...
		m_oGraph = oGraphFactory.CreateGraph(eDirectedness, eRestrictions);
	...
	)
    {
		...
		{
			IEdge oContainedEdge = oEdgeFactory.CreateEdge(
		...
		IEdge oNonContainedEdge = oEdgeFactory.CreateEdge(
		...
		for (Int32 i = 1; i < iVerticesToAdd; i++)
		{
			...
			switch (eAddOverload)
			{
				case AddOverload.IEdge:
					...
					IEdge oAddedEdge = m_oEdgeCollection.Add(oEdge);

					Assert.IsNotNull(oAddedEdge);

					Assert.AreEqual(oAddedEdge, oEdge);

					break;

				case AddOverload.IEdgeFactory:

					oEdge = m_oEdgeCollection.Add(new EdgeFactory(),
						oVertex1, oVertex2, bDirected);
