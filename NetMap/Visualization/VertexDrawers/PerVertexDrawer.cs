...
public class PerVertexDrawer : VertexDrawer
{
    ...
    )
	{
		...
		if ( TryGetPerVertexAlpha(vertex, out iPerVertexAlpha) )
		{
    ...
    protected Boolean
    TryGetPerVertexAlpha
    (
        IVertex oVertex,
		out Int32 iPerVertexAlpha
    )
	{
		Debug.Assert(oVertex != null);
		AssertValid();

		iPerVertexAlpha = Int32.MinValue;

		Object oPerVertexAlphaAsObject;

		if ( oVertex.TryGetValue(ReservedMetadataKeys.PerAlpha,
			typeof(Int32), out oPerVertexAlphaAsObject) )
		{
			iPerVertexAlpha = (Int32)oPerVertexAlphaAsObject;

			if (iPerVertexAlpha < 0 || iPerVertexAlpha > 255)
			{
				throw new FormatException( String.Format(
