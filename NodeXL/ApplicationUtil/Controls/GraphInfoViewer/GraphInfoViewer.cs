...
///
/// <remarks>
/// This control is meant to be used in conjunction with a NodeXLControl.  It
/// has one ListView that displays information about the vertices in the
/// NodeXLControl's graph, and another that displays information about the
/// graph's edges.  The user can select one or more vertices and edges by
/// clicking on the NodeXLControl, which causes the corresponding ListView items
...
/// NodeXLControl.  Context menus on the ListViews provide additional options.
///
/// <para>
/// Initialize this control by setting its <see cref="Graph" /> property.  When
/// the user changes the selected vertices and edges in the NodeXLControl, call
...
/// and edges in the NodeXLControl.
/// </para>
///
/// <para>
...
public partial class GraphInfoViewer : UserControl
{
	...
	)
	{
		...
		if (m_bFiringRequestSelection)
		{
			...
			// GraphInfoViewer is telling the NodeXLControl to change its
			// selection and the NodeXLControl is letting the GraphInfoViewer
			// know that the selection has changed.  The selection state of the
			// ListView items is already in sync with the NodeXLControl, so
			// ignore the event.
	...
	OnSelectedIndexChanged()
    {
		...
		// NodeXLControl.  To prevent this, fire the RequestSelection event from
	...
	protected static readonly String Int32Format = NodeXLBase.Int32Format;

	/// String format for Singles.

	protected static readonly String SingleFormat = NodeXLBase.SingleFormat;
