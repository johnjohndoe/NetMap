...
+ "\r\n\r\n"
+ "The third option is to specify a \"source column\" containing numbers you want NodeXL to use to automatically calculate edge widths.  Let's say you have a numerical Tie Strength column on the Edges worksheet.  If you open the AutoFill Columns dialog box, select \"Tie Strength\" from the Edge Width drop-down, and click Show Graph in the graph pane, NodeXL will calculate a width for each edge based on the edge's Tie Strength, fill in the Width column, and use the calculated widths in the graph.  (By default, NodeXL calculates the widths using a linear mapping between the Tie Strength numbers and the range of available widths, so that the edge with the smallest Tie Strength gets the smallest width and the edge with the largest Tie Strength gets the largest width.  You can modify this behavior by clicking the Edge Width Options button in the AutoFill Columns dialog box.)"
+ "\r\n\r\n"
