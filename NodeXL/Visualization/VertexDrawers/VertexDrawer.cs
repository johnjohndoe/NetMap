...
public class VertexDrawer : VertexDrawerBase, IVertexDrawer
{
    ...
    )
	{
		...
		//
		// NodeXLControl uses a two-bitmap scheme in which a selected vertex is
    ...
    //
    /// <overloads>
    /// Draws an outline around a rectangular vertex.
    /// </overloads>
	///
    /// <summary>
    ...
    )
	{
		...
		DrawVertexOutline(oDrawContext, oOutlineRectangle,
			bDrawSelected ? this.SelectedColor : this.Color, bDrawSelected);
	}
    ...
    /// This method can be used by derived classes that draw vertices as
	/// rectangles.  It draws an outline around the specified rectangle.  The
	/// outline is drawn with <see cref="SelectedOutlinePenWidth" /> if the
	/// vertex should be drawn as selected, or <see cref="OutlinePenWidth" />
	/// if not.
    /// </remarks>
    //*************************************************************************
	...
	///
	/// <para>
	/// If the <see cref="ReservedMetadataKeys.Visibility" /> key is present
	/// and is not set to <see cref="VisibilityKeyValue.Visible" />, false is
	/// returned even if a <see cref="HitTestArea" /> object was stored in
	/// the vertex's metadata.
	/// </para>
	///
	/// </remarks>
	//*************************************************************************
	...
	)
	{
		...
		if (
			GetVisibility(oVertex) == VisibilityKeyValue.Visible
			&&
			oVertex.TryGetValue(ReservedMetadataKeys.HitTestArea,
